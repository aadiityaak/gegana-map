...
class JibomIncidentsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $columns = [
            'description' => $schema->hasColumn('jibom_incidents', 'description'),
            'photos' => $schema->hasColumn('jibom_incidents', 'photos'),
            'latitude' => $schema->hasColumn('jibom_incidents', 'latitude'),
            'longitude' => $schema->hasColumn('jibom_incidents', 'longitude'),
            'news_source' => $schema->hasColumn('jibom_incidents', 'news_source'),
            'news_url' => $schema->hasColumn('jibom_incidents', 'news_url'),
        ];
        $now = now();
        ...
        $galleryPaths = $columns['photos'] ? $this->ensureGallerySeedImages('jibom') : [];
        if ($columns['photos'] && count($galleryPaths) > 0) {
            $this->backfillGalleryPhotos('jibom_incidents', $galleryPaths, $now);
        }
        ...
        $currentCounts = DB::table('jibom_incidents')
    ...
    private function makeIncidentRow(
        ...
        if (($columns['news_source'] ?? false) || ($columns['news_url'] ?? false)) {
            ...
            $newsUrl = $newsSource === 'online'
                ? $this->randomNewsUrl('jibom', $incidentType)
                : null;
        }
